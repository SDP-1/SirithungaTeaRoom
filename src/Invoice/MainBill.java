...
public class MainBill implements Printable {

    private static final DecimalFormat df = new DecimalFormat("0.00");
    //    private ObservableList<OrderDeatalsTM> list;
    private ObservableList<SaleTableTM> list;
    private OrderDetails orderDetails;

    public MainBill() {
    }

    //    public Bill(OrderDetails orderDetails ,ObservableList<OrderDeatalsTM> list) {
    public MainBill(OrderDetails orderDetails, ObservableList<SaleTableTM> list) {
        this.list = list;
...
//                g2d.drawImage(icon.getImage(), 50, 20, 90, 30, rootPane);y+=yShift+30;
                ...
                g2d.drawString(" Date       - " + orderDetails.getDate(), 10, y);
                y += yShift;
                g2d.drawString(" Time       - " + orderDetails.getTime(), 10, y);
                y += yShift;
                g2d.drawString("----------------------------------------", 10, y);
                y += headerRectHeight;
                g2d.drawString("        QTY       Price       Amount", 10, y);
                y += yShift;
                g2d.drawString("----------------------------------------", 10, y);
                y += headerRectHeight;

                for (int i = 0; i < list.size(); i++) {
                    SaleTableTM tm = list.get(i);
                    g2d.drawString(" " + tm.getNo() + " " + tm.getDescription(), 10, y);
                    y += yShift;
                    g2d.drawString("      " + df.format(tm.getQty()), 20, y);
                    g2d.drawString("      " + df.format(tm.getPrice()), 75, y);
                ...
                g2d.drawString("---------------------------------------", 10, y);
                y += yShift;
                g2d.drawString(" QTY          :  " + df.format(orderDetails.getNoOfItem()) + "   ", 10, y);
                y += yShift;
                g2d.drawString(" Paid statment:  " + (orderDetails.isOnLoan() ? "Loan" : "Paid") + "   ", 10, y);
                y += yShift;
                y += yShift;
...
//                g2d.drawString("-------------------------------------",10,y);y+=yShift;
                ...
                g2d.drawString(" Cash         :  " + df.format(orderDetails.getCash()) + "   ", 10, y);
                y += yShift;
//                g2d.drawString("-------------------------------------",10,y);y+=yShift;
                g2d.drawString(" Balance      :  " + df.format(orderDetails.getBalance()) + "   ", 10, y);
                y += yShift;
